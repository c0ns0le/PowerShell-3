...
#>
    ...
    $psAuth = $response | ? {$_.app.name -Like "*$AuhorizationName*"}
...
#>
    ...
    try {
        ...
        Write-Verbose "Invoke-RestMethod response:`n$(ConvertTo-Json $response1)"
        $sha = $response1.sha
    } catch {
        $sha = $null
    }
    ...
    if ($Force -or $PSCmdlet.ShouldProcess($fullpath)) {
        ...
        try {
            Write-Verbose "Invoke-RestMethod params:`n$(ConvertTo-Json $params)"
            $response2 = Invoke-RestMethod @params
            Write-Verbose "Invoke-RestMethod response:`n$(ConvertTo-Json $response2)"
        } catch {
            $errmsg  = "`nURL: $url"
            $errmsg += "`nERROR: " + $Error[0].Exception.Message
            Write-Host $errmsg -ForegroundColor Red
